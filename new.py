...
# #             pnl = np.zeros(n_rows, dtype=float)
# #             net_qty = np.zeros(n_rows, dtype=int)
# #             exit_time = pd.Series([pd.NaT] * n_rows, dtype="str").to_numpy()
# #             remain = np.abs(qty).astype(int)
...
# #     carry_fwd_pos_df = new_df[new_df["Net_Quantity"] != 0].copy()
# #     ###########################################################################
# #     carry_fwd_pos_df["Quantity"] =  carry_fwd_pos_df["Net_Quantity"]
# #     ###########################################################################
# #     new_df.to_csv(output_final_path, index=False)
# #     carry_fwd_pos_df.to_csv(output_carry_path, index=False)
...
#         for symbol in user_df["Symbol"].unique():
#             sym_df = (
#                 user_df[user_df["Symbol"] == symbol]
#                 .copy()
...
#             n = len(sym_df)
#             qty = sym_df["Quantity"].astype(int).to_numpy()
#             price = sym_df["Avg Price"].astype(float).to_numpy()
#             txn_time = sym_df["Exchange Time"].to_numpy()

#             pnl = np.zeros(n, dtype=float)
#             net_qty = np.zeros(n, dtype=int)
#             exit_time = np.full(n, pd.NaT, dtype="str")

#             # Unified FIFO queue
#             # item = [row_index, signed_remaining_qty, entry_price]
#             q = deque()

#             for i in range(n):
#                 cur_qty = qty[i]               # signed
#                 cur_rem = abs(cur_qty)

#                 # =========================
#                 # BUY → close shorts first
#                 # =========================
#                 if cur_qty > 0:
#                     while cur_rem > 0 and q and q[0][1] < 0:
#                         o_idx, o_rem, o_px = q[0]
#                         matched = min(cur_rem, -o_rem)

#                         pnl[i] += (o_px - price[i]) * matched

#                         cur_rem -= matched
#                         o_rem += matched

#                         if o_rem == 0:
#                             exit_time[o_idx] = txn_time[i]
#                             q.popleft()
#                         else:
#                             q[0][1] = o_rem

#                     if cur_rem > 0:
#                         q.append([i, cur_rem, price[i]])
...
#             # Remaining open positions → carry forward
#             for idx, rem, _ in q:
#                 net_qty[idx] = rem

#             sym_df["PNL"] = pnl
#             sym_df["Net_Quantity"] = net_qty
#             sym_df["Exit_time"] = exit_time

#             total_realized_pnl += pnl.sum()
#             final_df = pd.concat([final_df, sym_df], ignore_index=True)

#     # Carry-forward positions
#     carry_fwd_df = final_df[final_df["Net_Quantity"] != 0].copy()
#     carry_fwd_df["Quantity"] = carry_fwd_df["Net_Quantity"].abs().astype(int)

#     # Save outputs
#     final_df.to_csv(output_final_path, index=False)
#     carry_fwd_df.to_csv(output_carry_path, index=False)

#     st.write(f"[{exchange}] Total Realized PNL: **{total_realized_pnl:,.2f}**")

#     return final_df, carry_fwd_df
...
#                     st.session_state.today = date.today().strftime("%Y%m%d")
...
# # Always show download section if results exist in session state
...
#     today = st.session_state.today if 'today' in st.session_state else date.today().strftime("%Y%m%d")
#     output_name = st.session_state.output_name if 'output_name' in st.session_state else "updated_positions.csv"
...
#     if st.button("Clear Results", key="clear_tab0"):
#         if 'download_data' in st.session_state:
#             del st.session_state.download_data
#         if 'output_name' in st.session_state:
#             del st.session_state.output_name
#         if 'today' in st.session_state:
#             del st.session_state.today
...
def handle_carryforward(df, carry_path, users, exchange, enable_carry, broker_type):
    ...
    try:
        ...
        if carry_df.empty:
            st.info(f"[{exchange}] No carry positions for these users")
            return df
        carry_df=carry_df.sort_values(by='Order Time')
        # carry_df["Order Time"] = pd.to_datetime(carry_df["Order Time"], format="%d-%b-%Y %H:%M:%S", errors="coerce")
        # df["Order Time"] = pd.to_datetime(df["Order Time"], format="%d-%b-%Y %H:%M:%S", errors="coerce")     
        if broker_type == "Noren":
            df = pd.concat([carry_df, df], ignore_index=True) # first order of carry_df then today orders
...
# =============================================================================
#  Tab 1: Noren Bhavcopy Carryforward Update
#  (full logic with both features: update Avg Price and calculate/display/add Unrealized PNL)
# =============================================================================
...
def move_to_downloads_with_updated_name(src_path, original_name, suffix="_updated"):
    if not src_path or not os.path.exists(src_path):
        return None, None
    downloads = os.path.join(os.path.expanduser("~"), "Downloads")
    os.makedirs(downloads, exist_ok=True)
        ...
        if (has_nfo_carry and not has_nfo_bhav) or (has_bfo_carry and not has_bfo_bhav):
            ...
            with st.status("Updating carry-forward files...", expanded=True) as status:
                ...
                if "carry_nfo" in temp_paths:
                    ...
                    try:
                        df_nfo = pd.read_csv(temp_paths["carry_nfo"])
                        required_cols = ["Symbol", "Avg Price", "Transaction", "Quantity", "User ID"]
                        missing_cols = [col for col in required_cols if col not in df_nfo.columns]
                        if missing_cols:
                            st.error(f"NFO carry file missing columns: {', '.join(missing_cols)}.")
                        else:
                            df_nfo["Symbol_clean"] = clean_symbol(df_nfo["Symbol"])
                            if price_map_nfo:
                                # Calculate unrealized PNL before updating Avg Price
                                df_nfo["Original Avg Price"] = df_nfo["Avg Price"]  # Temporary column for original
                                df_nfo["New Price"] = df_nfo["Symbol_clean"].map(price_map_nfo)  # Get new price
                                df_nfo["Diff"] = 0.0
                                buy_mask = df_nfo["Transaction"].str.upper() == "BUY"
                                sell_mask = df_nfo["Transaction"].str.upper() == "SELL"
                                df_nfo.loc[buy_mask, "Diff"] = df_nfo.loc[buy_mask, "New Price"] - df_nfo.loc[buy_mask, "Original Avg Price"]
                                df_nfo.loc[sell_mask, "Diff"] = df_nfo.loc[sell_mask, "Original Avg Price"] - df_nfo.loc[sell_mask, "New Price"]
                                df_nfo["Unrealized PNL"] = df_nfo["Diff"] * df_nfo["Quantity"].abs()

                                # Group by User ID and display total unrealized PNL
                                pnl_summary_nfo = df_nfo.groupby("User ID")["Unrealized PNL"].sum().reset_index()
                                st.subheader("NFO Total Unrealized PNL per User")
                                st.dataframe(pnl_summary_nfo)

                                # Update Avg Price
                                updated_count = df_nfo["New Price"].notna().sum()
                                df_nfo["Avg Price"] = df_nfo["New Price"].fillna(df_nfo["Avg Price"])
                                st.success(f"NFO: Updated {updated_count} positions with settlement price.")

                                # Save verification file (with calculations for verification)
                                verif_cols = ["User ID", "Symbol", "Transaction", "Quantity", "Original Avg Price", "New Price", "Diff", "Unrealized PNL"]
                                df_verif_nfo = df_nfo[verif_cols].copy()
                                df_verif_nfo.to_csv(temp_paths["carry_nfo"] + "_verif", index=False)
                                verif_path, verif_name = move_to_downloads_with_updated_name(temp_paths["carry_nfo"] + "_verif", carry_nfo_upl_tab1.name, suffix="_pnl_verification")
                                if verif_path:
                                    verification_files.append((verif_name, verif_path))

                            else:
                                st.warning("No valid NFO bhavcopy data → Avg Price unchanged. No PNL calculated.")

                            df_nfo.drop(columns=["Symbol_clean", "Original Avg Price", "New Price", "Diff"], errors="ignore", inplace=True)

                            # Overwrite temp file with updated content (keep Unrealized PNL if calculated)
                            df_nfo.to_csv(temp_paths["carry_nfo"], index=False)
                            out_path, out_name = move_to_downloads_with_updated_name(temp_paths["carry_nfo"], carry_nfo_upl_tab1.name)
                            if out_path:
                                updated_files.append((out_name, out_path))
                ...
                if "carry_bfo" in temp_paths:
                    ...
                    try:
                        df_bfo = pd.read_csv(temp_paths["carry_bfo"])
                        required_cols = ["Symbol", "Avg Price", "Transaction", "Quantity", "User ID"]
                        missing_cols = [col for col in required_cols if col not in df_bfo.columns]
                        if missing_cols:
                            st.error(f"BFO carry file missing columns: {', '.join(missing_cols)}.")
                        else:
                            df_bfo["Symbol_clean"] = clean_symbol(df_bfo["Symbol"])
                            if price_map_bfo:
                                # Calculate unrealized PNL before updating Avg Price
                                df_bfo["Original Avg Price"] = df_bfo["Avg Price"]  # Temporary column for original
                                df_bfo["New Price"] = df_bfo["Symbol_clean"].map(price_map_bfo)  # Get new price
                                df_bfo["Diff"] = 0.0
                                buy_mask = df_bfo["Transaction"].str.upper() == "BUY"
                                sell_mask = df_bfo["Transaction"].str.upper() == "SELL"
                                df_bfo.loc[buy_mask, "Diff"] = df_bfo.loc[buy_mask, "New Price"] - df_bfo.loc[buy_mask, "Original Avg Price"]
                                df_bfo.loc[sell_mask, "Diff"] = df_bfo.loc[sell_mask, "Original Avg Price"] - df_bfo.loc[sell_mask, "New Price"]
                                df_bfo["Unrealized PNL"] = df_bfo["Diff"] * df_bfo["Quantity"].abs()

                                # Group by User ID and display total unrealized PNL
                                pnl_summary_bfo = df_bfo.groupby("User ID")["Unrealized PNL"].sum().reset_index()
                                st.subheader("BFO Total Unrealized PNL per User")
                                st.dataframe(pnl_summary_bfo)

                                # Update Avg Price
                                updated_count = df_bfo["New Price"].notna().sum()
                                df_bfo["Avg Price"] = df_bfo["New Price"].fillna(df_bfo["Avg Price"])
                                st.success(f"BFO: Updated {updated_count} positions with settlement price.")

                                # Save verification file (with calculations for verification)
                                verif_cols = ["User ID", "Symbol", "Transaction", "Quantity", "Original Avg Price", "New Price", "Diff", "Unrealized PNL"]
                                df_verif_bfo = df_bfo[verif_cols].copy()
                                df_verif_bfo.to_csv(temp_paths["carry_bfo"] + "_verif", index=False)
                                verif_path, verif_name = move_to_downloads_with_updated_name(temp_paths["carry_bfo"] + "_verif", carry_bfo_upl_tab1.name, suffix="_pnl_verification")
                                if verif_path:
                                    verification_files.append((verif_name, verif_path))

                            else:
                                st.warning("No valid BFO bhavcopy data → Avg Price unchanged. No PNL calculated.")

                            df_bfo.drop(columns=["Symbol_clean", "Original Avg Price", "New Price", "Diff"], errors="ignore", inplace=True)
            ...
            if updated_files:
                st.success(f"Updated files saved to your Downloads folder! (Includes updated Avg Price and Unrealized PNL column if calculated)")
                for fname, fpath in updated_files:
                    if os.path.exists(fpath):
                        with open(fpath, "rb") as f:
                            st.download_button(
            ...
            if verification_files:
                st.success(f"PNL Verification files saved to your Downloads folder!")
                for fname, fpath in verification_files:
                    if os.path.exists(fpath):
                        with open(fpath, "rb") as f:
                            st.download_button(
                                label=f"📥 Download {fname} (for PNL verification)",
                                data=f,
                                file_name=fname,
                                mime="text/csv",
                                key=f"dl_verif_{fname}"
                            )
            ...
            for p in temp_paths.values():
                if p and os.path.exists(p):
                    try:
                        ...
                        pass
                # Also clean up _verif temps if exist
                verif_temp = p + "_verif" if p else None
                if verif_temp and os.path.exists(verif_temp):
                    try:
                        os.unlink(verif_temp)
                    except:
                        pass
